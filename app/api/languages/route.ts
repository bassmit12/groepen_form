...
export async function GET() {
  try {
    ...
    const response = await fetch(`${baseUrl}/${database}/languages`, {
      method: "GET",
      headers: {
    ...
    if (!response.ok) {
      ...
      return NextResponse.json(
        { error: `Error fetching languages: ${response.statusText}` },
    ...
    console.error("Error in languages API route:", error);
    return NextResponse.json(
      {
